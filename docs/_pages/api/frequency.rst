...
The flowchart below visualizes the structure of this function. The NNI series are interpolated at a new sampling frequency of 4Hz before the PSD computation is conducted as the unevenly sampled NNI series would distort the PSD.
...
PSD methods. Define the compatible input parameters in Python dictionaries and pass it to the ``kwargs`` input
