...
**Input Parameters**
   ...
   - ``duration`` (int, optional): Maximum duration per segment in [s] (default: 300s)
...
First, the input NNI series is segmented into segments of ~5min in duration. Second, the SDNN parameter of each segment is computed. Finally, the mean value of all computed SDNN values is computed.
...
First, the input NNI series is segmented into segments of ~5min in duration. Second, the mean of each segment is computed. Finally, the SDNN value of all computed mean values is computed.
...
.. hint::
   ...
   Other than the ``nn50()`` and the ``NN20()`` functions which derive the NNXX parameters based on 50ms and 20ms
   threshold, you can use this function to apply custom thresholds.
...
with:
   ...
   * :math:`pnnXX`: Ratio of NNI differences > 50ms and :math:`n`
   * :math:`nnXX`: Number of NNI differences > 50ms
   * :math:`n`: Number of NNI differences
...
with:
   ...
   * :math:`pnnXX`: Ratio of NNI differences > 20ms and :math:`n`
   * :math:`nnXX`: Number of NNI differences > 20ms
   * :math:`n`: Number of NNI differences
...
Additionally, the ``pyhrv.time_domain.geometrical_parameters()`` function allows you to compute all geometrical parameters and to join them in a single NNI histogram with using only a single function.
...
Module Level Function: time_domain()
####################################
